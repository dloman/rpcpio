...
// Use the finest unit whose value fits in 8 digits, moving to a coarser unit
// only when it represents the duration exactly. Values are rounded up so the
// peer never sees a shorter deadline than the caller asked for.
std::string FormatTimeout(std::chrono::nanoseconds duration) {
    ...
    for (std::size_t i = 0; i < std::size(kUnits); ++i) {
        ...
        const std::uint64_t val = (ns + u.ns_per - 1) / u.ns_per;
        if (val > kMaxTimeoutValue) continue;
        const bool has_coarser = i + 1 < std::size(kUnits);
        if (has_coarser && ns % kUnits[i + 1].ns_per == 0) continue;
