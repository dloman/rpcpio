...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    auto decoded = rpcpio::metadata::DecodeBase64(s);
    // Re-encode must produce valid base64 that decodes back to the same bytes.
    auto re_encoded = rpcpio::metadata::EncodeBase64(decoded);
    auto re_decoded = rpcpio::metadata::DecodeBase64(re_encoded);
    (void)re_decoded;
    return 0;
}
